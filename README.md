...
- **Smart auto-stop + chapter markers** — near the scheduled end, DVarr watches the live match **status** (never the scoreline) and *extends* the recording in 15-minute steps while play continues. Extra time and penalty shoot-outs land in the file — even at 0-0 — and a race ending is never trimmed. The same live tracking embeds **chapters** (Kick-off, Half-time, Extra time, Penalty shoot-out, Full time) into every finished recording, readable natively by Plex, Jellyfin, Kodi and VLC.
...
- **Delete-after-watched (Plex / Jellyfin webhook)** — the retention **"delete after watched"** mode (Settings → Storage, set globally or per-league) needs your media server to tell DVarr when you've finished a recording. DVarr exposes a **token-secured** inbound webhook for exactly that — copy the ready-made URL from **DVarr → Settings → Storage** (it ends in `…/api/webhooks/jellyfin?token=…`; the token is **required** or the request is rejected). If the shown URL is relative, prepend your DVarr address (e.g. `http://<dvarr-lan-ip>:1867`).
  - **Jellyfin** — install the official **Webhook** plugin, then **Add Generic Destination** — the JSON one, *not* "Generic Form" (that posts form fields; DVarr reads JSON):
    1. **Webhook URL** → the `…/api/webhooks/jellyfin?token=…` URL from Settings → Storage.
    2. **Notification Type** → tick **both** **Playback Stop** *and* **User Data Saved** (Playback Stop catches watching a recording to the end; User Data Saved catches marking one watched by hand — DVarr acts on either).
    3. Enable **Send All Properties (ignores template)** so `PlayedToCompletion` / `Played` and the file path all reach DVarr. Save. (The plugin's own top-of-page **"Server Url"** field is for links inside notifications — it is *not* where DVarr's URL goes.)
  - **Plex** (Plex Pass) — **Settings → Webhooks → Add Webhook** → the `…/api/webhooks/plex?token=…` URL. DVarr acts on Plex's `media.scrobble` event (~90% watched).
  - Watch a recording to the end, then check **DVarr → Logs** for `[Webhook] … marked watched`. DVarr matches by file path or, when the webhook sends no path (common on Jellyfin's "mark watched"), by **series + season + episode**, so a differing mount path between the two still resolves. Until a league (or the global default) is on the "delete after watched" mode, DVarr just records the watched flag and deletes nothing.
